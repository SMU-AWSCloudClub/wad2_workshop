    ...
    <div class="row justify-content-center">
      <div class="col-12 col-md-8 col-lg-6">
        ...
        <form @submit.prevent="addPost" class=" p-4 rounded">
          ...
          <!-- <div class="form-group col-md-6 mx-auto w-100 mb-1">
            <input type="file" @change="handleImageUpload" class="">
          </div> -->
...
export default {
  ...
  methods: {
    ...
    //   // Call AWS Lambda to upload 
    //   const { url } = await uploadToS3(file);

    //   this.newPost.image = url;
    // },

    async fetchPosts() {
      this.loading = true
      const response = await axios.get('https://be.bchwy.com/2read.php')
      this.posts = response.data
      this.loading = false
    },

    async addPost() {
      const response = await axios.post('https://be.bchwy.com/1create.php', this.newPost, {
        headers: {
    ...
    async deletePost(id) {
      await axios.post('https://be.bchwy.com/4delete.php', { id }, {
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
        }
      });
    ...
    async updatePost(updatedPost) {
      await axios.post('https://be.bchwy.com/3update.php', updatedPost, {
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
        }
      });
